...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Update Corner {{ $corner->name }}</h1>
</div>
<div class="col-6">
    <a href="/admin/corner" class="btn btn-sm btn-warning mb-3">Back</a>
    <form action="/admin/corner/{{ $corner->id }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        {{-- @dd($corner) --}}
        <div class="mb-3">
            <label class="form-label">Corner's name</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $corner->name) }}" placeholder=" ">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('detail') is-invalid @enderror" name="detail" id="detail">{{ old('detail', $corner->detail) }}</textarea>
            @error('detail')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('location') is-invalid @enderror" name="location" value="{{ old('location', $corner->location) }}">
            @error('location')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                <input type="time" class="form-control @error('jam_buka') is-invalid @enderror" name="jam_buka" value="{{ old('jam_buka', $corner->jam_buka) }}">
                <span class="input-group-text"> until </span>
                <input type="time" class="form-control @error('jam_tutup') is-invalid @enderror" name="jam_tutup" value="{{ old('jam_tutup', $corner->jam_tutup) }}">
                @error('jam_buka')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="mb-3">
            ...
            <div class="row">
                ...
                @foreach ($days as $day)
                    <div class="col-3">
                        <input type="checkbox" id="day{{ $loop->index }}" name="days[]" value="{{ $day }}" {{ in_array($day, old('days', $corner->hari_buka) ?: []) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <div class="row">
                @foreach ($categories as $item)
                    <div class="col-3">
                        <input type="checkbox" id="c{{ $item->id }}" name="categories[]" value="{{ $item->id }}" {{ in_array($item->id, old('categories', $corner->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <div class="row">
                @foreach ($facilities as $item)
                    <div class="col-3">
                        <input type="checkbox" id="f{{ $item->id }}" name="facilities[]" value="{{ $item->id }}" {{ in_array($item->id, old('facilities', $corner->facilities->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                <input type="number" class="form-control @error('harga_min') is-invalid @enderror" name="harga_min" placeholder="Minimum budget" value="{{ old('harga_min', $corner->harga_min) }}">
                <span class="input-group-text"> - </span>
                <input type="number" class="form-control @error('harga_max') is-invalid @enderror" name="harga_max" placeholder="Maximum budget" value="{{ old('harga_max', $corner->harga_max) }}">
                @error('harga_min')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
